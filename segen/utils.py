...
def goal_rewarding(action,goal,cur_sequence:list,goal_indicator:int=0,goals:list=[],flag_test:bool=False,mode:str='train'):
    """
    given a sequence, when the last function is the target function, give reward and terminate (assume the sequence is valid in training and testing)
    (in prediction mode, the valididty of the sequence is not guaranteed as there is no idea to know the validity of the sequence)
    """
    reward=0
    terminate = False
